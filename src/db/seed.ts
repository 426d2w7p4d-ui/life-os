...
export async function ensureSeedData() {
  ...
  await db.courses.bulkAdd([
    { id: uid(), name: '大学英语', teacher: '李老师', room: 'A302', color: '#6f86ff', weekday, startTime: '14:00', endTime: '15:30', startWeek: 1, endWeek: 18, weekType: '全部', examDate: toLocalISO(exam), createdAt: now, updatedAt: now },
    { id: uid(), name: '计算机基础', teacher: '王老师', room: 'B205', color: '#7b9b8b', weekday: weekday === 5 ? 2 : weekday + 1, startTime: '10:00', endTime: '11:40', startWeek: 1, endWeek: 18, weekType: '全部', createdAt: now, updatedAt: now }
  ]);

  await db.countdowns.add({ id: uid(), name: '英语四级考试', date: toLocalISO(exam), icon: 'GraduationCap', color: '#d58a54', category: '考试', repeat: '不重复', mode: '倒计时', createdAt: now });
  ...
  const petId = uid();
  await db.pets.add({ id: petId, name: '团子', species: '猫', breed: '英短', sex: '公', weight: 5.2, neutered: true, birthday: '2023-04-12', notes: '性格安静', createdAt: now, updatedAt: now });
  await db.petRecords.add({ id: uid(), petId, type: '驱虫', title: '体外驱虫', date: today, nextDate: toLocalISO(tomorrow), createdAt: now });
